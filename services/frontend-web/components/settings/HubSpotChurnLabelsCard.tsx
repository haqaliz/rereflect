...
const REASON_COPY: Record<string, string> = {
  unknown_pipeline: 'Unknown pipeline selected — one or more ids are not recognized by HubSpot.',
  no_active_integration: 'No active HubSpot integration.',
  options_fetch_failed: 'Could not fetch live pipelines from HubSpot right now.',
  missing_read_scope: 'HubSpot token is missing read permission for pipelines.',
...
const STATUS_COPY: Record<string, string> = {
  ...
  'error: missing_read_scope': 'HubSpot token is missing read permission',
};
...
interface HubSpotChurnLabelsCardProps {
  status: HubSpotConnectionStatus;
  onStatusChange: (status: HubSpotConnectionStatus) => void;
}

export function HubSpotChurnLabelsCard({ status, onStatusChange }: HubSpotChurnLabelsCardProps) {
  const [selected, setSelected] = useState<string[]>(
    status.churn_label_config?.renewal_pipeline_ids ?? []
  );
  ...
  // HubSpotWritebackCard.tsx's inputDisabled precedent).
  ...
  const loadOptions = async () => {
    ...
    try {
      const res = await hubspotAPI.getChurnLabelOptions();
      setOptions(res.options);
    } catch (err: any) {
      const detail = err?.response?.data?.detail;
      const reason = typeof detail === 'object' ? detail?.reason : null;
      setOptionsError(friendlyReason(reason) || 'Could not fetch live pipelines.');
    } finally {
  ...
  const handleToggle = async (checked: boolean) => {
    ...
    setSaving(true);
    try {
      await hubspotAPI.updateChurnLabels({
        enabled: checked,
        config: checked ? { renewal_pipeline_ids: selected } : null,
      });
      const refreshed = await hubspotAPI.getStatus();
      onStatusChange(refreshed);
    } catch (err: any) {
  ...
  const handleRunBackfill = async () => {
    ...
    try {
      await hubspotAPI.triggerChurnBackfill(backfillMonths);
      const refreshed = await hubspotAPI.getStatus();
      onStatusChange(refreshed);
    } catch (err: any) {
      const detail = err?.response?.data?.detail;
  ...
  const handleCancelBackfill = async () => {
    ...
    setBackfillActionLoading(true);
    try {
      await hubspotAPI.cancelChurnBackfill();
      const refreshed = await hubspotAPI.getStatus();
      onStatusChange(refreshed);
    } catch (err: any) {
      const detail = err?.response?.data?.detail;
  ...
  return (
    <Card className="animate-slide-up">
      <CardHeader>
        ...
        <CardDescription>
          Suggest churn labels from lost HubSpot renewal deals, reviewed by an operator before
          they ever train the calibrator.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Only lost deals in the renewal pipelines you pick below are ever suggested.
            </p>
          </div>
          <Switch
        ...
        <div className="space-y-2">
          <p className="text-sm font-medium">Renewal pipelines</p>
          <Popover open={popoverOpen} onOpenChange={handlePopoverOpenChange}>
            ...
            <PopoverContent>
              ...
              {!optionsLoading && !optionsError && options && options.length === 0 && (
                <p className="text-sm text-muted-foreground">HubSpot returned no deal pipelines.</p>
              )}
              {!optionsLoading && !optionsError && options && options.length > 0 && (
                <div className="space-y-2">
                  {options.map((opt) => (
                    <div key={opt.id} className="flex items-center gap-2">
                      ...
                      />
                      <label htmlFor={`pipeline-${opt.id}`} className="text-sm cursor-pointer">
                        {opt.label}
                      </label>
                    </div>
                  ))}
        ...
        {showEmptyStateWarning && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              No renewal pipelines selected — no suggestions will be created. Pick the pipeline
              your renewals close in.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="space-y-3 pt-2 border-t border-border">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Run a one-time pass over closed-lost deals from up to 5 years back — years of
              lost renewals land in the review queue as suggestions, never labels.
            </p>
          </div>
