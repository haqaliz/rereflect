...
describe('SalesforceChurnLabelsCard', () => {
  ...
  describe('Backfill history (historical-backfill aspect)', () => {
    ...
    it('Run is disabled when the renewal set is empty even if enabled', () => {
      const emptyEnabled: SalesforceConnectionStatus = {
        ...baseStatus,
        churn_labels_enabled: true,
        churn_label_config: { renewal_opportunity_types: [] },
      };
      render(<SalesforceChurnLabelsCard status={emptyEnabled} onStatusChange={vi.fn()} />);
      expect(screen.getByRole('button', { name: /^run$/i })).toBeDisabled();
    });
    ...
    it('shows Cancel instead of Run while a backfill is running', () => {
      const runningStatus: SalesforceConnectionStatus = {
        ...enabledStatus,
        backfill_status: 'running',
        backfill_progress: { scanned: 10, suggested: 2, skipped_existing: 1 },
      };
      render(<SalesforceChurnLabelsCard status={runningStatus} onStatusChange={vi.fn()} />);
    ...
    it('clicking Cancel calls cancelChurnBackfill and refreshes status', async () => {
      ...
      render(<SalesforceChurnLabelsCard status={runningStatus} onStatusChange={onStatusChange} />);
    ...
    it('renders backfill_error', () => {
      const failedStatus: SalesforceConnectionStatus = {
        ...enabledStatus,
        backfill_status: 'failed',
        backfill_error: 'missing_encryption_key',
      };
      render(<SalesforceChurnLabelsCard status={failedStatus} onStatusChange={vi.fn()} />);
    ...
    it('shows an explicit dropped-by-cap warning naming the count and covered window — no silent caps', () => {
      const cappedStatus: SalesforceConnectionStatus = {
        ...enabledStatus,
      ...
      render(<SalesforceChurnLabelsCard status={cappedStatus} onStatusChange={vi.fn()} />);
    ...
    it('does not show the dropped-by-cap warning when nothing was dropped', () => {
      const okStatus: SalesforceConnectionStatus = {
        ...enabledStatus,
        backfill_status: 'completed',
        backfill_progress: { scanned: 10, suggested: 10, dropped_by_cap: 0 },
      };
      render(<SalesforceChurnLabelsCard status={okStatus} onStatusChange={vi.fn()} />);
