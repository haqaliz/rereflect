...
import { salesforceAPI, type SalesforceConnectionStatus } from '@/lib/api/salesforce';

const DEFAULT_FIELD_NAME = 'Rereflect_Health_Score__c';
...
// plus the soft `ambiguous_contact` note the push task leaves in
// last_writeback_error when it wrote to a best-guess Contact match.
const REASON_COPY: Record<string, string> = {
  field_not_found: 'Field not found in Salesforce.',
  wrong_type: 'Field exists but is not a writable number field.',
  missing_write_scope: 'Salesforce token is missing write permission for this field.',
  ambiguous_contact: 'Written to a best-guess match — multiple matching contacts were found for this email.',
  validation_error: 'Could not validate the field right now. Please try again.',
};
...
const STATUS_COPY: Record<string, string> = {
  ...
  field_not_found: 'Field not found in Salesforce',
  contact_not_found: 'Contact not found in Salesforce',
  retrying: 'Retrying after a transient error',
  'error: missing_write_scope': 'Salesforce token is missing write permission',
...
interface SalesforceWritebackCardProps {
  status: SalesforceConnectionStatus;
  onStatusChange: (status: SalesforceConnectionStatus) => void;
}

export function SalesforceWritebackCard({ status, onStatusChange }: SalesforceWritebackCardProps) {
  ...
  const handleToggle = async (checked: boolean) => {
    ...
    setSaving(true);
    try {
      await salesforceAPI.updateWriteback({
        enabled: checked,
  ...
  const handleValidate = async () => {
    ...
    setTestResult(null);
    try {
      const res = await salesforceAPI.testWriteback(trimmed);
      setTestResult(res);
    } catch (err: any) {
      setTestResult({
        ok: false,
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Automatically write the health score to Salesforce whenever it changes.
            </p>
          </div>
          <Switch
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            The Salesforce Contact field to write the health score into.
        ...
        {status.last_writeback_error && (
          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>{friendlyReason(status.last_writeback_error)}</AlertDescription>
          </Alert>
        )}
      </CardContent>
    </Card>
