...
import { UrgentFeedbackPageProvider, useUrgentFeedbackPage } from '@/contexts/UrgentFeedbackPageContext';

function UrgentFeedbackPageContent() {
  const router = useRouter();
  const { searchQuery, currentPage, setSearchQuery, setCurrentPage } = useUrgentFeedbackPage();
  ...
  const fetchFeedback = async () => {
    try {
      ...
      const firstPage = await feedbackAPI.list(1, 100, { is_urgent: true });
      ...
      if (totalPages > 1) {
        ...
        for (let page = 2; page <= totalPages; page++) {
          pagePromises.push(feedbackAPI.list(page, 100, { is_urgent: true }));
        }
  ...
  const filterFeedback = () => {
    let filtered = feedbackList.filter(item => item.is_urgent);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-text-secondary font-medium">Loading urgent feedback...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="animate-fade-in">
          <div className="flex items-center space-x-3 mb-2">
            <div className="p-3 bg-red-100 dark:bg-red-900/30 rounded-xl">
              <CircleAlert className="w-8 h-8 text-red-700 dark:text-red-400" />
            </div>
            <div>
              <h1 className="text-4xl font-bold text-text-primary">Urgent Feedback</h1>
              <p className="text-text-secondary text-lg">Critical issues requiring immediate attention</p>
            </div>
          </div>
        </div>
        ...
        <Card className="p-6 animate-slide-up stagger-1">
          <div className="relative">
            ...
            <Input
              type="text"
              placeholder="Search urgent feedback..."
              value={searchQuery}
          ...
          <p className="text-sm text-muted-foreground mt-4 font-mono">
            Showing <span className="font-semibold text-foreground">{filteredList.length}</span> urgent feedback items
          </p>
        </Card>
                      ...
                      <TableCell className="max-w-md">
                        <div className="flex items-start space-x-3">
                          <div className="flex-shrink-0 mt-1">
                            <div className="p-2 bg-red-100 dark:bg-red-900/30 rounded-lg border border-red-200 dark:border-red-800/50">
                              <CircleAlert className="w-4 h-4 text-red-700 dark:text-red-400" />
                            </div>
                          </div>
                          <div className="flex-1">
                            <p className="leading-relaxed mb-2">{item.text}</p>
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="h-48 text-center">
                      <div className="flex flex-col items-center justify-center space-y-4">
                        <CircleAlert className="w-16 h-16 text-muted-foreground opacity-20" />
                        <div>
                          <h3 className="text-lg font-medium mb-2">No urgent feedback found</h3>
                          <p className="text-muted-foreground">
                            {searchQuery
                              ? 'Try adjusting your search query'
                              : 'No urgent feedback at the moment'}
                          </p>
                        </div>
                      </div>
                    </TableCell>
...
export default function UrgentFeedbackPage() {
  return (
    <UrgentFeedbackPageProvider>
      <UrgentFeedbackPageContent />
    </UrgentFeedbackPageProvider>
  );
}
