...
import { PainPointsPageProvider, usePainPointsPage } from '@/contexts/PainPointsPageContext';

function PainPointsPageContent() {
  const router = useRouter();
  const { searchQuery, currentPage, setSearchQuery, setCurrentPage } = usePainPointsPage();
  ...
  const fetchFeedback = async () => {
    try {
      ...
      const firstPage = await feedbackAPI.list(1, 100, { sentiment: 'negative' });
      ...
      if (totalPages > 1) {
        ...
        for (let page = 2; page <= totalPages; page++) {
          pagePromises.push(feedbackAPI.list(page, 100, { sentiment: 'negative' }));
        }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="flex flex-col items-center space-y-4">
          ...
          <p className="text-text-secondary font-medium">Loading pain points...</p>
        </div>
      </div>
    );
  }
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="animate-fade-in">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div>
              <h1 className="text-4xl font-bold text-text-primary">Top Pain Points</h1>
              <p className="text-text-secondary text-lg">Customer issues and problems that need attention</p>
            </div>
          </div>
        </div>
        ...
        <Card className="p-6 animate-slide-up stagger-1">
          ...
          <p className="text-sm text-muted-foreground mt-4 font-mono">
            Showing <span className="font-semibold text-foreground">{filteredList.length}</span> pain points
          </p>
        </Card>
                      ...
                      <TableCell className="max-w-md">
                        <div className="flex items-start space-x-3">
                          <div className="flex-shrink-0 mt-1">
                            <div className="p-2 bg-orange-100 dark:bg-orange-900/30 rounded-lg border border-orange-200 dark:border-orange-800/50">
                  ...
                  <TableRow>
                    <TableCell colSpan={6} className="h-48 text-center">
                      <div className="flex flex-col items-center justify-center space-y-4">
                        ...
                        <div>
                          <h3 className="text-lg font-medium mb-2">No pain points found</h3>
