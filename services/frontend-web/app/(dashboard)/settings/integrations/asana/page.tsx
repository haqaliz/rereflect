...
export default function AsanaSettingsPage() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const s = await asanaAPI.getStatus();
      setStatus(s);
    } catch {
      // ignore
    }
  ...
  const handleConnect = async () => {
    if (!tokenInput.trim()) {
      setConnectError('Please enter your Asana Personal Access Token.');
      return;
    }
    setConnecting(true);
    ...
    setTestResult(null);
    try {
      const result = await asanaAPI.connect({
        api_token: tokenInput.trim(),
      });
      setStatus({
        connected: result.connected,
        token_hint: result.token_hint,
        account_gid: result.account_gid,
      ...
      const msg: string =
        err?.response?.data?.detail ?? 'Failed to connect Asana. Check your Personal Access Token.';
      setConnectError(msg);
    } finally {
      setConnecting(false);
    }
  ...
  const handleTest = async () => {
    ...
    setTestResult(null);
    try {
      const res = await asanaAPI.testConnection();
    ...
    <div className="min-h-screen pattern-bg">
      <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="animate-fade-in">
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-[#F06A6A]/10">
              <AsanaIcon className="w-7 h-7 text-[#F06A6A]" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-foreground">Asana</h1>
              <p className="text-muted-foreground text-sm">
                Connect Asana to create tasks directly from feedback.
              </p>
            </div>
            {status?.connected && (
              <Badge
        ...
        <Card className="animate-slide-up">
          <CardHeader>
            <CardTitle>
              {status?.connected ? 'Connection Details' : 'Connect Asana'}
            </CardTitle>
            <CardDescription>
              {status?.connected
                ? 'Your Asana account is connected. You can test or disconnect below.'
                : 'Create a Personal Access Token in Asana and paste it below.'}
            </CardDescription>
          </CardHeader>
            ...
            {status?.connected ? (
              ...
              isAdminOrOwner ? (
                <div className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      <code className="font-mono">app.asana.com/0/my-apps</code>.
                    </p>
                  </div>
                ...
                <p className="text-sm text-muted-foreground">
                  Contact an admin to connect Asana.
                </p>
              )
            )}
          </CardContent>
        ...
        <Card className="animate-slide-up stagger-1">
          <CardHeader>
            <CardTitle className="text-base">How to get your Personal Access Token</CardTitle>
          </CardHeader>
          <CardContent>
            <ol className="text-sm text-muted-foreground space-y-1 list-decimal list-inside">
              <li>Go to <strong>app.asana.com</strong> → My Settings → Apps</li>
              <li>Click <strong>Manage Developer Apps</strong></li>
              <li>Click <strong>Create new token</strong></li>
              <li>Name the token and copy it</li>
              <li>Paste the token above and click <strong>Connect Asana</strong></li>
            </ol>
          </CardContent>
        </Card>
      </main>
      ...
      <Dialog open={confirmDisconnect} onOpenChange={(open) => { if (!open) setConfirmDisconnect(false); }}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Disconnect Asana?</DialogTitle>
            <DialogDescription>
              This will remove the Asana connection. Existing linked tasks will not be deleted.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
