"""
TDD migration tests for per-org-churn-model (churn-predict-seam-resolver,
data-and-config aspect).

Same strategy as test_org_ai_config_urgency_mode_migration.py: build a
pre-migration SQLite schema (org_ai_config WITH classifier_mode AND
category_classifier_mode AND urgency_classifier_mode, WITHOUT the churn
columns — i.e. the state right after f6a7b8c9d0e1), apply this migration's
upgrade()/downgrade() via the Alembic Operations proxy directly, and inspect
with sqlalchemy.inspect().

Revision target: ``3cb9a0d1456b`` (slug: add_churn_classifier_mode).
down_revision: ``f6a7b8c9d0e1`` (confirmed single head via `alembic heads`
at authoring time).
"""
...
_migration = importlib.import_module("3cb9a0d1456b_add_churn_classifier_mode")
...
class TestMigrationRevisionPointer:
    def test_revision_and_down_revision(self):
        assert _migration.revision == "3cb9a0d1456b"
        assert _migration.down_revision == "f6a7b8c9d0e1"


class TestChurnClassifierModeColumns:
    def test_columns_added_after_upgrade(self, migrated_engine):
        insp = inspect(migrated_engine)
        col_map = {c["name"]: c for c in insp.get_columns("org_ai_config")}
        assert "churn_classifier_mode" in col_map, "churn_classifier_mode column missing"
        assert col_map["churn_classifier_mode"]["nullable"], "must be nullable"
        assert "churn_autopromote_hold" in col_map, "churn_autopromote_hold column missing"
        assert col_map["churn_autopromote_hold"]["nullable"], "must be nullable"
    ...
    def test_existing_sibling_columns_untouched(self, migrated_engine):
        ...
        insp = inspect(migrated_engine)
        col_map = {c["name"]: c for c in insp.get_columns("org_ai_config")}
        for sibling in ("classifier_mode", "category_classifier_mode", "urgency_classifier_mode"):
            assert sibling in col_map
            assert col_map[sibling]["nullable"]
    ...
    def test_server_default_off(self, migrated_engine):
        with migrated_engine.begin() as conn:
            ...
            row = conn.execute(text(
                "SELECT churn_classifier_mode, churn_autopromote_hold "
        ...
        assert row[1] in (False, 0, "false")


class TestMigrationDowngrade:
    def test_downgrade_removes_churn_columns_only(self):
        engine = _make_engine()
        _pre_meta.create_all(bind=engine)
        ...
        insp = inspect(engine)
        after_upgrade = {c["name"] for c in insp.get_columns("org_ai_config")}
        assert "churn_classifier_mode" in after_upgrade
        assert "churn_autopromote_hold" in after_upgrade
        ...
        assert "churn_classifier_mode" not in remaining
        assert "churn_autopromote_hold" not in remaining
        ...
        assert "urgency_classifier_mode" in remaining, "downgrade must not remove the urgency column"
