"""
TDD tests for registering `jira` as a selectable feedback-source type.

Jira brings its own auth (like Linear) so it must be:
...
def free_org(db: Session) -> Organization:
    org = Organization(name="Free Jira Source Org", plan="free")
    db.add(org)
    db.commit()
    db.refresh(org)
    return org
...
class TestJiraSourceTypeListed:

    def test_types_endpoint_includes_jira(self, client: TestClient, free_user_headers: dict):
    ...
    def test_jira_entry_requires_no_integration(self, client: TestClient, free_user_headers: dict):
        response = client.get("/api/v1/feedback-sources/types", headers=free_user_headers)
        jira_entry = next(e for e in response.json() if e["type"] == "jira")
        assert jira_entry["requires_integration"] is False

    def test_jira_entry_is_available(self, client: TestClient, free_user_headers: dict):
        response = client.get("/api/v1/feedback-sources/types", headers=free_user_headers)
        jira_entry = next(e for e in response.json() if e["type"] == "jira")
        assert jira_entry["available"] is True


class TestCreateJiraFeedbackSource:

    def test_free_plan_org_can_create_jira_source(self, client: TestClient, free_user_headers: dict):
        """No plan gate blocks `jira` source creation on a FREE-plan org."""
        response = client.post(
            "/api/v1/feedback-sources/",
            json={"source_type": "jira", "name": "My Jira Source"},
            headers=free_user_headers,
        )
        assert response.status_code == 201

    def test_created_jira_source_has_expected_fields(self, client: TestClient, free_user_headers: dict):
        response = client.post(
            "/api/v1/feedback-sources/",
            json={"source_type": "jira", "name": "My Jira Source"},
            headers=free_user_headers,
