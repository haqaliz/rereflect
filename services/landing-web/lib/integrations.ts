...
export const integrations: Integration[] = [
  ...
  },
  {
    ...
    description: 'Connect a Teams webhook and let Rereflect post alerts — urgent feedback, health drops, sentiment spikes, automation and playbook notifications — straight to the Teams channel where your team works.',
    status: 'available',
    color: 'chart-4',
    gradient: 'from-[#6264A7] to-[#8B8CC7]',
    hoverShadow: 'hover:shadow-[#6264A7]/10',
    hoverBorder: 'hover:border-[#6264A7]/30',
    heroMessage: 'Your team already lives in Teams. Rereflect meets you there: connect a webhook once, and alerts about urgent feedback, customer health drops, sentiment spikes and churn risk land in the channel your team actually watches.',
    howItWorks: [
      ...
      { step: '2', title: 'Paste the URL into Rereflect', description: 'Settings → Integrations → Microsoft Teams. Rereflect validates the URL and sends a test message to confirm the channel receives it.' },
    ...
    features: [
      ...
      { title: 'MessageCard Formatting', description: 'Alerts render as Microsoft MessageCards with Rereflect\u2019s accent color — the format Teams webhooks accept natively.', icon: 'MessageSquare' },
    ],
    useCases: [
    ...
    faqs: SHARED_FAQS,
    setupSteps: [
      { step: 1, title: 'Create an Incoming Webhook in Teams', description: 'In your Teams channel, go to the "..." menu → Connectors → Incoming Webhook, name it (e.g. "Rereflect Alerts"), and copy the webhook URL. Power Automate "Post to a channel when a webhook request is received" Workflows URLs work too.' },
      { step: 2, title: 'Go to Settings → Integrations', description: 'Navigate to your Rereflect dashboard and open Settings → Integrations → Microsoft Teams.' },
      { step: 3, title: 'Paste the webhook URL', description: 'Paste the URL and click Connect. Rereflect validates the format (classic `outlook.office.com/webhook/…` or `webhook.office.com/webhookb2/…` Workflows URLs) and rejects invalid ones on save.' },
      { step: 4, title: 'Send a test message', description: 'Rereflect posts a test card to the channel so you can confirm delivery before relying on it.' },
  ...
  },
  {
    ...
    howItWorks: [
      { step: '1', title: 'Connect Intercom', description: 'Create a private app in the Intercom Developer Hub and paste its access token into Settings → Integrations → Intercom. No OAuth app registration or environment variables needed — Rereflect validates the token and provisions your feedback source automatically.' },
      { step: '2', title: 'Conversations Flow In', description: 'New conversations are pulled every 15 minutes; replies and ratings arrive in near-real time when you subscribe the optional webhook. Each conversation becomes one feedback item, however it arrives.' },
      ...
      { step: '4', title: 'Close the Loop (opt-in)', description: 'Turn on the write-back and resolving a feedback item adds a note to its Intercom conversation and closes it — your support team sees the outcome where they work.' },
    ],
    features: [
      { title: 'Conversation Sync', description: 'New conversations are pulled automatically every 15 minutes, and an optional webhook adds replies and ratings in near-real time — no webhook subscription is required to get started.', icon: 'MessageCircle' },
      ...
      { title: 'Close the Loop', description: 'Opt-in write-back: resolve a feedback item and Rereflect adds a note to the conversation and can close it — off by default, note-only also available.', icon: 'RefreshCw' },
      { title: 'Webhook Integration', description: 'Optional real-time webhook with per-workspace HMAC signature verification; the 15-minute pull keeps working with or without it.', icon: 'Shield' },
    ],
    useCases: [
    ...
    faqs: SHARED_FAQS,
    setupSteps: [
      { step: 1, title: 'Create a private app', description: 'Open the Intercom Developer Hub, create a private app, and copy its Access Token — plus the Client Secret if you plan to use real-time webhooks.' },
      { step: 2, title: 'Paste the token in Rereflect', description: 'Go to Settings → Integrations → Intercom, paste the Access Token, and click Connect. Rereflect validates it and provisions your feedback source automatically.' },
      { step: 3, title: 'Conversations start flowing', description: 'New conversations are pulled every 15 minutes and become analyzed feedback items — nothing else to configure.' },
      { step: 4, title: 'Add the webhook for real-time delivery (optional)', description: 'Subscribe conversation.user.created, conversation.user.replied and conversation.rating.added in your Intercom app. Without a Client Secret, webhook deliveries are rejected — the pull keeps working.' },
      { step: 5, title: 'Enable the write-back (optional)', description: 'On the Intercom settings page, turn on Write-back to post a note and close the conversation when you resolve a feedback item. Requires conversation:write scope on your Intercom app.' },
      { step: 6, title: 'Watch conversations become feedback', description: 'New conversations, replies, and ratings appear on your dashboard — with sentiment, categories, and churn signals.' },
    ],
  },
  {
    slug: 'email',
