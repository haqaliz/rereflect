...
def _upsert_sidecar(db, feedback_id: int, status: str) -> None:
    ...
    to UPDATE). If a concurrent writer — the poll task and a webhook
    delivery both observing the SAME ticket for the first time — reaches
    this at the same instant, both may see `row is None` here and both
    attempt to INSERT the same feedback_id PK. The insert is isolated in a
    SAVEPOINT (mirrors the race-safe insert pattern already used elsewhere
    in this codebase, e.g. usage_webhooks.py) so the loser's IntegrityError
    only unwinds this nested transaction, not the whole task's session; the
    loser then treats the winner's already-committed row as authoritative
    and does nothing further — equivalent to Postgres `ON CONFLICT
    (feedback_id) DO NOTHING`, but dialect-agnostic (works against both the
    sqlite dialect used in tests and Postgres in production). This path is
    only ever reached on the "seed" action (decide_update only returns
    "seed" when no sidecar row existed), so no status change or event is
    ever at stake for the loser here.
    """
