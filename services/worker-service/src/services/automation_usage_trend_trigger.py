...
It deliberately does NOT mirror the whole engine. Its action set is exactly
`HANDLED_ACTION_TYPES` (pinned to the backend support matrix); anything else
is recorded as an explicit error.
...
# SUPPORTED_ACTIONS_BY_TRIGGER["usage_trend"] — pinned by
...
def _execute_run_playbook_actions(
    ...
    `run_playbook`, `send_customer_email` and `send_notification` — same scope
    as `automation_churn_trigger._execute_run_playbook_actions`, with the same
    shared handlers. Every other action type is recorded as an explicit
    "Unsupported action type" error entry.
    """
...
def _execute_send_notification(
    ...
    if not notify_config.get("message_template"):
        notify_config["message_template"] = (
            f"Automation '{rule.name}' triggered (usage trend) for customer {customer_email}"
        )
