...
class UserAlertPreference(Base):
    ...
    channel_teams = Column(Boolean, default=True, nullable=False)
...
class OrgAIConfig(Base):
    ...
    # Per-org CHURN-classifier mode (mirrors backend-api;
    # per-org-churn-model, churn-predict-seam-resolver). 'off' | 'shadow' |
    # 'auto'. Independent of the sentiment/category/urgency mode columns.
...
# ── Scheduled AI Reports (scheduled-ai-reports) ─────────────────────────────
# DUPLICATED: mirrors services/backend-api/src/models/report.py (lightweight,
# no FKs — worker model convention). report_metadata maps to the DB column
# "metadata" exactly as the backend mirror does.
class Report(Base):
    """Saved AI-generated report for an organization — worker mirror."""
    ...
    report_metadata = Column("metadata", JSON, nullable=True)  # {schedule_id, source, generated_at, date_start, date_end, model_used}
...
# DUPLICATED: mirrors services/backend-api/src/models/report_schedule.py
# (lightweight, no FK constraints — worker model convention).
class ReportSchedule(Base):
    """Scheduled AI report configuration for an organization — worker mirror."""
