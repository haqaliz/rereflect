...
end-to-end (worker-service), through REAL resolve_classifier +
...
contract with the backend-api suite).
"""
...
from src.models import OrgAIConfig, OrgClassifierModel
...
class TestClassifierSeamMatrixEndToEnd:
    ...
    def test_seam_case_real_stack(self, case, db, test_org, caplog):
        _setup(
            db, test_org.id,
        ...
        feedback.organization_id = test_org.id
