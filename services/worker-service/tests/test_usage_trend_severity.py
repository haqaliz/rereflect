"""
TDD tests for the worker-service DUPLICATE of the usage-trend severity
ordering helper (worker-trend-evaluator aspect) — strict TDD (RED first).

This file is intentionally near-identical to
services/backend-api/tests/test_usage_trend_severity.py — the module under
test is a byte-for-byte logic copy (see the "DUPLICATED: keep in sync"
header on src/services/usage_trend_severity.py), so its test coverage
should be too, to catch drift between the two copies.

Run:
    cd services/worker-service && ./venv/bin/python -m pytest tests/test_usage_trend_severity.py -v
"""
...
# guard case insufficient_history -> sharp_decline (PRD E3 / AC2).
# ---------------------------------------------------------------------------
