...
artifact defense + per-org cache (worker-service).

Mirrors probability_updater._load_active_model / _deserialize_model
defensiveness, adapted for OrgClassifierModel (M5.2 predict-seam-resolver).
Aspect B's predict()/score_from_proba() are NOT exercised here (Phase 3) —
this phase only covers the load/cache/deserialize seam.
"""
...
from src.models import Organization, OrgClassifierModel
