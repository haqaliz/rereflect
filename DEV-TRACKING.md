...
- [ ] **Deferred (v2):** OAuth 3LO, Jira Server/Data Center, **inbound webhook (real-time)** (poll-based status-sync shipped), per-status-**name** mapping editor UI, outbound `feedback.status_changed` webhook on a Jira-driven change, multiple sites per org
