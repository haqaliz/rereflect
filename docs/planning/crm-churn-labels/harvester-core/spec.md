...
- `churn_labels_enabled` / `churn_label_config` columns, PATCH + options endpoints, UI — `org-config-api-and-ui` (M3).
...
- **Before** `review-queue` (M5), which consumes the rows, and `historical-backfill` (M7), which
...
- **R4 (regression) — the live one.** Harvest runs inside the load-bearing enrichment loop.
  Mitigation: additive call site, exception-isolated, enrichment output characterized
...
- **Sync duration.** Harvest adds closed-lost fetches to a 03:15/03:45 beat; the per-run cap bounds
  it and the per-account memoization (`salesforce_sync.py:251-252`) is reused. If it still stalls
  the beat, split it out — do not raise the cap silently.
