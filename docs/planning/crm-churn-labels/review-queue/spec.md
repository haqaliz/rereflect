...
1. **`GET /api/v1/customers/churn-suggestions`** — `require_admin_or_owner` +
   ...
   `provider=hubspot|salesforce`, `search` (email substring). Returns `{items, total, page,
   page_size}` — items are whole suggestion rows (incl. `evidence`, `churn_event_id`), ordered
   `suggested_churned_at DESC, id DESC`.
2. **`POST /api/v1/customers/churn-suggestions/{id}/confirm`** — same deps. Body
   ...
   (Finding 6). Writes, via the **existing service path**, `CustomerChurnEvent(org, customer_email,
   churned_at=suggestion.suggested_churned_at, reason_code, reason_text,
   marked_by_user_id=current_user.id, source='manual')`; sets
...
5. **Collision handling (non-negotiable).** `UNIQUE(organization_id, customer_email, churned_at)`
   (`models/churn_event.py:87-96`): confirming for a customer already hand-marked at the same close
   date raises `IntegrityError`. Confirm MUST (a) pre-check `_has_existing_active_event` →
   ...
   partial-bulk abort. A `skipped` suggestion still **resolves** (`status='confirmed'`,
...
8. **Frontend.** `lib/api/churn-suggestions.ts` — `listChurnSuggestions`, `confirmChurnSuggestion`,
   ...
   navigating to the review view; zero pending → card absent (an unconfigured org sees nothing, per
   PRD personas). Review rows show `customer_email` + evidence (deal name, close date,
   ...
   **required-reason-code UX** (`ReasonCodeSelect`; submit disabled until picked), with the
   churned-date input **replaced by a read-only CRM close date** (stability makes re-harvest
   idempotent, PRD M2). Toast states skips: `"41 confirmed, 6 skipped (already marked)"`.

## Out of scope

- The suggestions table + migration, harvester, provider fetch, opt-in config (waves 1–2); backfill (M7); readiness counting (M6).
- Pre-existing churn-event route gaps (Finding 5): missing role deps, inconsistent dedup across the
  three write paths, `recover`/`delete` not invalidating probability, `RecoverRequest.note`
  discarded, and the never-built 500/day rate limit (our writes are human-gated).
...
8. Bulk: duplicate email → deduped, `matched` counts it once; both `{emails}` and `{filter}` (or
   neither) → 422; `confirm` without `reason_code` → 422, nothing written; a `filter` cohort touches
   only `pending` rows of the caller's org.
...
- **Blocks:** M6 readiness copy ("N pending suggestions") reads the same status counts; precision
  (confirmed ÷ reviewed) is only computable once this ships.
- **Parallel-safe with:** M1/M2/M4 (worker-side) — no shared files. New backend files:
  ...
  `src/api/main.py`. Tests: house style — real SQLite `db` fixture, no Celery, no patching but AC5.
...
- **R-A — the feature's whole trust boundary is one endpoint.** If confirm ever writes without a
  ...
  or timeline — Finding 3). **Mitigation:** `marked_by_user_id` asserted (AC3), never NULL; no
  auto-confirm in v1.
- **R-B — `skipped` semantics could mis-attribute.** Resolving a collision to
  ...
  did not create. **Mitigation:** toast and `results[].reason` say `already_marked`; leaving it
  `pending` forever would re-surface an unactionable row daily.
- **R-C — bulk over a `filter` cohort is unbounded** (the 500/day cap was never built).
  **Mitigation:** cap the resolved cohort per request, reporting `matched` vs. cap — **no silent
  caps**; value set in the plan.
- **R-D — evidence is the operator's only basis for judgement**; a thin/null blob makes confirm a
  coin-flip. **Mitigation:** render an explicit "no CRM detail captured" state, not an empty cell.
- **R-E — reason-code overloading.** CRM suggestions map to none of the six codes (Finding 6), so
  operators reach for `other`, diluting a bucket the fit filters on. Accepted for v1 per the PRD.
