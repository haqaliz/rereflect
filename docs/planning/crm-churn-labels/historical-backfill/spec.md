...
with reviewing and leaves the gate unreachable. The volume lives in the CRM's *history* — years of
closed-lost renewals already sitting there, harvestable in one bounded pass.
...
4. **Paginated + throttled, both providers** — Salesforce `queryMore` (`nextRecordsUrl` until
   ...
   backfill cannot exhaust the org's CRM quota.
5. **The shipped error taxonomy, unchanged** — 429 → `Retry-After` sleep → `*TransientError` →
   `self.retry`; 5xx → transient, no sleep; 403 → scope error, no retry; SF 401 → refresh once;
   **never** touches `is_active`. No new exception types.
6. **Resumable + idempotent** via `UNIQUE(organization_id, provider, external_opportunity_id)`
...
7. **Progress + no silent caps (house rule)** — persist `backfill_status`
   (`idle|running|completed|failed|cancelled`), `backfill_progress` (pages/records scanned,
   created, skipped-as-duplicate), `backfill_last_run_at`, `backfill_error` on the integration
   row. Cap truncation → dropped count **logged AND surfaced** with the window actually covered.
8. **Trigger endpoint** — `POST /api/v1/integrations/{provider}/churn-labels/backfill`,
   `require_admin_or_owner`, org-scoped, dispatched via `send_task` (backend cannot import worker
   code). **Broker failure → 502, not 500.** Rejects when `churn_labels_enabled` is false or
   config is empty (default-deny) → 400; run already `running` → 409.
9. **Settings-card progress** — the M3 provider card gains a "Backfill history" control (window
   picker + Run) rendering status/progress/dropped-count/error from §7.
...
- Auto-running backfill on enable, or auto-confirming backfilled suggestions.
- Backfilling `crm_enrichment` or any non-churn history.
...
- **AC-5 (idempotent + resumable).** Running the same window twice creates the suggestion once;
  the second run reports it skipped-as-duplicate, not created, and raises **no** `IntegrityError`.
  A run aborted after page 1 then re-run completes the window with no duplicates. A `rejected`
  suggestion is not recreated.
...
- **AC-9 (cancellable).** Cancel mid-run stops before the next page, persists
  `backfill_status='cancelled'` with partial progress; already-written suggestions stay.

**Test style.** Strict TDD, tests first. Real SQLite `db` fixture + a hand-written multi-page Fake
client injected through the M4 client seam (precedent: `FakeJiraClient`). No Celery, no httpx, no
...
- **R1 (PRD) — volume may still not reach 500.** Backfill harvests what exists. We surface honest
  counts and promise no org reaches the gate; per R8 the target is itself under review, so this
  aspect's value is threshold-independent.
- **R3 (PRD) — historical rows are the least verifiable.** A 3-year-old lost renewal is hardest to
  confirm from memory. Mitigated by M4's `evidence` payload on every row, and by never
  auto-confirming.
- **Long-running task vs. worker restarts.** AC-5 resumability makes re-running safe and cheap, so
  a lost run is never a data problem.
- **R4 (PRD) — paging edits touch shipped client code.** Sibling accessors only; M1's enrichment
  characterization lock must stay green.
