...
> LLM_ENCRYPTION_KEY is not set; refusing to leave Linear webhook secrets in plaintext. Generate a Fernet key (e.g. python -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"), set LLM_ENCRYPTION_KEY, and re-run `alembic upgrade head`.
> ```
>
...
> afterwards invalidates every stored token and webhook secret — already true for
> all integrations, so pick a key once and keep it.
