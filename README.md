...
| 💬 **AI Copilot (Cmd+K)** | Ask questions in natural language, generate reports and draft responses — including **recurring scheduled reports with optional email delivery** (BYO-key Resend). Runs **fully offline on a local model** (Ollama or any OpenAI-compatible endpoint) — including the template-matching embeddings — or on your BYOK cloud key. |
